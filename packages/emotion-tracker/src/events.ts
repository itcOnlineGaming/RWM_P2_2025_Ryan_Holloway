...
export function persistSessionStart(payload: EmotionData & { userId?: string }) {
...
export function querySessionsByUser(userId: string) {
  ...
  return sessions.sort((a: any, b: any) => b.sessionStartTimestamp - a.sessionStartTimestamp);
}

export function querySessionsByTimeRange(userId: string, start: number, end: number) {
  const sessions = getSessions(userId);
  return sessions.filter((s: any) => s.sessionStartTimestamp >= start && s.sessionStartTimestamp <= end);
}

// Simple analytics counter for emotion selections over time
const ANALYTICS_PREFIX = 'emotion_tracker_analytics_';
export function logEmotionSelection(userId: string, emotion: PreSessionEmotion) {
