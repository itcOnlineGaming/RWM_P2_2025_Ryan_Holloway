 ...
 */
export function getPerformanceInsight(emotion, analytics) {
    ...
    if (diff > 0) {
        return `When feeling ${emotion}, you perform ${Math.round(diff)}% better than average.`;
    }
    else {
        return `When feeling ${emotion}, you perform ${Math.round(Math.abs(diff))}% worse than average.`;
    }
}
/**
